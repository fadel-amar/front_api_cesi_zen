<template>
  <div v-if="visible" class="fixed inset-0 backdrop-blur-sm bg-black/10 flex items-center justify-center z-50"
    ...
    <div
      ...
      <h2 class="text-xl sm:text-2xl font-bold text-gray-800">Nouvelle catégorie</h2>
...
watch(
  () => props.apiErrors,
  (newApiErrors) => {
    errors.value = { name: '', emoji: '', duration: '' }
    if (newApiErrors) {
...
const handleSubmit = () => {
  ...
  if (!formData.value.name) {
    ...
    errors.value.name = 'Le nom doit avoir moins de 50 caractères'
    hasErrors = true
  }

  if (!formData.value.emoji) {
    errors.value.emoji = 'Un emoji doit être sélectionné'
    hasErrors = true
  }
